...
import com.mycompany.order.config.EmbeddedKafka;
import com.mycompany.order.config.KafkaSseConsumer;
import com.mycompany.order.config.KafkaSseProducer;
...
class OrderServiceKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/order-service-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/order-service-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/order-service-kafka/unregister"));
                return;
            }
        }
