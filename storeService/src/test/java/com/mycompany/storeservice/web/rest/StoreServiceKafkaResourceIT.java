...
import com.mycompany.storeservice.config.EmbeddedKafka;
import com.mycompany.storeservice.config.KafkaSseConsumer;
import com.mycompany.storeservice.config.KafkaSseProducer;
...
class StoreServiceKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/store-service-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/store-service-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/store-service-kafka/unregister"));
                return;
            }
        }
