...
import com.mycompany.store.web.config.EmbeddedKafka;
import com.mycompany.store.web.config.KafkaSseConsumer;
import com.mycompany.store.web.config.KafkaSseProducer;
...
class WebStoreKafkaResourceIT {
    ...
    void producesMessages() throws InterruptedException {
        client.post().uri("/api/web-store-kafka/publish?message=value-produce").exchange().expectStatus().isNoContent();
    ...
    void consumesMessages() {
        ...
        String value = client
            .get()
            .uri("/api/web-store-kafka/consume")
